...
1. Connectors fetch source-specific payloads from Open-Meteo, NOAA/NWS, and IMD.
...
4. The connector normalizes the payload and publishes `weather.normalized.observation.v1`.
...
- The database bootstrap script uses `ON_ERROR_STOP` so extension/schema failures fail initialization rather than leaving a partially initialized database.
...
The TimescaleDB image is pinned to PostgreSQL 16.15 / TimescaleDB 2.30.0 and uses the maintained HA `all` image so the required PostGIS extension is present. The database healthcheck verifies both `timescaledb` and `postgis` before dependent services start.
...
| IMD | Implemented, configuration required | Uses the official IMD Current Weather API. Configure `WEATHERGPT_IMD_STATION_IDS` as a JSON object mapping configured city names to IMD station IDs. |
...
### IMD configuration example

```text
WEATHERGPT_IMD_STATION_IDS={"delhi":"42182"}
```

The official IMD API reference documents the Current Weather API at `https://api.imd.gov.in/api/v1/current_wx?id=StationId`. Station IDs are unique identifiers; do not invent mappings. Add additional city/station mappings from authoritative IMD station metadata as they are verified.
...
- `POST /ingest/current?city=mumbai` publishes raw and normalized Kafka events and returns `status=queued` for a configured source.
